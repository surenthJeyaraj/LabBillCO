    ...
    $.ajax({
        ...
        success: function (serverData) {
            ...
            if (serverData.Success === true) {
                //var validJson = doJsonValidate(serverData.OrderFormData, null);
...
function navTab(form_name, target) {
    ...
    var isValidForm = doValidate(form_name, '#VALIDATION_MESSAGES',"FORM_HL7");
    if (isValidForm) {
...
function checkAllForms() {
    ...
    $.each(formList, function (key, val) {
        ...
        $('#VALIDATION_MESSAGES').removeClass('alert alert-danger');
        $('#VALIDATION_MESSAGES').html('');
        $('<div class="col-md-6 validate-all" style="margin:10px auto" id="VALIDATION_MESSAGES' + key + '"> </div>').insertAfter("#VALIDATION_MESSAGES");
        
        var isValidForm = doValidate(val, '#VALIDATION_MESSAGES' + key);
        if (isValidForm == false) {
            var currTab = $('form[name=' + val + ']').closest('.tab-pane').attr('id');
            var tabTitle = $('.nav-tabs a[href="#' + currTab + '"]').html();
            $("<h5>" + tabTitle+"</h5>").prependTo('#VALIDATION_MESSAGES'+key);
            isAllValid = false;
        }
        else {
            $('#VALIDATION_MESSAGES' + key).remove();
...
AVUTOXPortal.User.populateAll = function (data) {
    ...
    if (data !=""){
        ...
        toastr.error("Scan Order has no data.");
...
function checkForSelfSubscriber() {
    if ($('form[name=in1-form] .subscriber-fields input[name=RelationshiptoSubscriber]:checked').val() == '800000000') {
        ...
        $('form[name=in2-form] .subscriber-fields input[name=RelationshiptoSubscriber][value=800000000]').attr('disabled', true);
        ...
        //****** FILLING PID FORM  DATAS*********
        $('form[name=in1-form] .subscriber-fields input[name=RelationshiptoSubscriber][value=800000000]').attr('disabled', true);
        ...
        $('form[name=in1-form] .subscriber-fields input[name=RelationshiptoSubscriber][value=800000000]').attr('disabled', false);
        $('form[name=in2-form] .subscriber-fields input[name=RelationshiptoSubscriber][value=800000000]').attr('disabled', false);
...
$(document).ready(function () {
    ...
    $('.nav-tabs .nav-item').on('click', function () {
        if (!$(event.target).closest('button').length) {
            ...
            var isValidForm = doValidate(activeForm, '#VALIDATION_MESSAGES',"FORM_HL7");
            $('.validate-all').remove();
            if (isValidForm) {
